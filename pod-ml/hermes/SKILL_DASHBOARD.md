...
It shows download progress, validation status, and log tails for GPM, ERA5, and the more_labels_1
group. **You only touch the dashboard when the user asks** — no proactive monitoring; just respond.

## Commands

All dashboard actions go through `podctl` (run as `sudo -u claude .../podctl`):
...
- Never edit any live file directly (not in `/home/claude/...`, not in `~/agent-work` outside the commit flow).
